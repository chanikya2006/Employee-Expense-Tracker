...
export class BusinessDataService {
  ...
  onGithub(){
    ...
    link.target="_blank";
    link.href="https://github.com/Anirudh-cheedella";
    link.click();
  }
  onLinkedin(){
    const link=document.createElement('a');
    link.target="_blank";
    link.href="https://www.linkedin.com/in/anirudh-cheedella-142268262/";
    link.click();
  }
...
getAllEmployees() {
  return this.http.get(this.apiUrl + 'EMPLOYEE/GET_ALL_EMPLOYEES');
}

addEmployee(employee: any) {
  return this.http.post(
    this.apiUrl + 'EMPLOYEE/ADD_EMPLOYEE',
    employee
  );
}
...
getEmployee(id: string) {
  return this.http.get(
    this.apiUrl + 'EMPLOYEE/GET_EMPLOYEE/' + id
  );
}

updateEmployee(id: string, employee: any) {
  return this.http.patch(
    this.apiUrl + 'EMPLOYEE/UPDATE_EMPLOYEE/' + id,
    employee
  );
}
...
deleteEmployee(id: string) {
  return this.http.delete(
    this.apiUrl + 'EMPLOYEE/DELETE_EMPLOYEE/' + id
  );
}
...
addEmployeeExpense(expense: any) {
  return this.http.post(
    this.apiUrl + 'EMPLOYEE_EXPENSE/ADD_EXPENSE',
    expense
  );
}
...
getEmployeeExpenses(employeeId: string) {
  return this.http.get(
    this.apiUrl +
    'EMPLOYEE_EXPENSE/GET_EMPLOYEE_EXPENSES/' +
    employeeId
  );
}

deleteEmployeeExpense(expenseId: string) {
  return this.http.delete(
    this.apiUrl +
...
getDashboardAnalytics() {
  return this.http.get(
    this.apiUrl + 'DASHBOARD/ANALYTICS'
  );
}
